...
;;; What is (q 5)?
;;; Answer: 10
;;; Explanation (what is adj bound to in each closure?): adj is 2 + 1 for p and 3 - 1 for q
...
;;; (check-grade 80) => 'upper
;;; (check-grade 40) => 'lower
;;; (check-age 12)   => 'upper
;;; (check-age 3)    => 'lower
;;; Explanation (what is mid bound to in each closure?): 50, 9
...
;;; Step by step:
;;; Line 1: k = 10
;;; Line 2-4: sum = a + b, f(x) = sum * (x + k)
;;; Line 5 (what frame(s) created? what is sum? what closure is returned?): sum is 5, frame is created, f(x) (a closure) is returned
;;; Line 6 (what frame(s) created? what is sum? what closure is returned?): sum is 2, frame is created, f(x) (a closure) is returned
;;; Line 7 (what frame created? what is the result?): f(0) = 5 * (0 + 10)
;;; Line 8 (what frame created? what is the result?): f(5) = 2 * (5 + 10)
;;;
;;; What is (fn1 0)?
;;; Answer: 50
;;; What is (fn2 5)?
...
;;; With lambda:
(map (lambda (x) (string-append x "!")) lst-4c)

;;; With curry or curryr:
(map (curryr string-append "!") lst-4c)
...
(define product-list (curry foldr * 1))
...
;;; With currying (point-free):
(define all-positive? (curry foldr (curryr > 0)))
