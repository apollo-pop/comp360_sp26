...
;;;
;;; Matching rules:
;;;   - The lexer always picks the LONGEST match
;;;   - Ties are broken by the FIRST rule listed
...
;;;
;;;   (lexer
;;;    [(from/to "(*" "*)")                               (next-token)]   ; Rule 1: ???
;;;    [(repetition 1 +inf.0 (char-range "0" "9")) (token 'NUM lexeme)]   ; Rule 2: ???
;;;    [(char-set "+-*")                            (token 'OP lexeme)]   ; Rule 3: ???
;;;    [whitespace                                        (next-token)]   ; Rule 4: ???
;;;    [any-char                                          (next-token)])  ; Rule 5: ???
;;;
;;; New forms used here:
  ...
  (define (next-token)
    (define minicalc-lexer
      (lexer
       ...
       ; the following lines of code go where they're supposed to!
...
;;;
;;; Replace any-char with your pattern and lexeme with your token:
...
;;;     and (next-token) for the action (to skip and keep going).
       ...
       [any-char lexeme] ; you can assume comments are always ended by a newline: "\n"
